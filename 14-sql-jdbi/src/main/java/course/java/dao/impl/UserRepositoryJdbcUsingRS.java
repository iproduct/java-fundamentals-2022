...
@Repository
@Slf4j
public class UserRepositoryJdbcUsingRS implements UserRepository {
    ...
    public UserRepositoryJdbcUsingRS(Connection connection) {
    ...
    private User insertUser(User user) throws SQLException {
        ...
        try (var stmt = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            stmt.execute(FIND_ALL_USERS, Statement.RETURN_GENERATED_KEYS);
            var rs = stmt.getResultSet();
            // move to insert row
            rs.moveToInsertRow();

            // update fields
            rs.updateString(2, user.getFirstName());
            rs.updateString(3, user.getLastName());
            rs.updateInt(4, user.getAge());
            rs.updateString(5, user.getPhone());
            rs.updateString(6, user.getUsername());
            rs.updateString(7, user.getPassword());
            rs.updateString(8, user.getRole().name());
            rs.updateBoolean(9, user.isActive());
            rs.updateTimestamp(10, Timestamp.valueOf(user.getCreated()));
            rs.updateTimestamp(11, Timestamp.valueOf(user.getModified()));

            // make insert
            rs.insertRow();

            // return back to current row
            rs.last();

            // get generated key
            user.setId(rs.getLong(1));
            log.info("Successfully INSERTED user ID: {}",user.getId());
            return user;
        } catch (Exception e) {
            log.error("Error inserting row using ResultSet", e);
            return null;
        }
    }
}
